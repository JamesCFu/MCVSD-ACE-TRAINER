...
interface LayoutProps { children: React.ReactNode; activeView: string; setActiveView: (view: string) => void; mistakeCount: number; }

const Layout: React.FC<LayoutProps> = ({ children, activeView, setActiveView, mistakeCount }) => { // Ref to control the scrolling element const mainContentRef = useRef<HTMLDivElement>(null); const [isSidebarOpen, setIsSidebarOpen] = useState(true);

// Scroll to top whenever the active view changes useEffect(() => { // Reset scroll on the main container if (mainContentRef.current) { mainContentRef.current.scrollTop = 0; } // Also reset window scroll just in case window.scrollTo(0, 0); }, [activeView]);

return ( <div className="min-h-screen flex flex-col md:flex-row bg-slate-50"> {/* Mobile Toggle (visible when sidebar is closed or on mobile) */} <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={fixed top-4 left-4 z-30 p-3 bg-indigo-950 text-white rounded-xl shadow-xl hover:bg-indigo-800 transition-all duration-300 ${isSidebarOpen ? 'md:hidden' : 'block'}} > <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 6h16M4 12h16M4 18h16"></path></svg> </button>
    ...
    <div className="p-6 flex flex-col h-full">
      ...
      <nav className="flex-1 space-y-1.5 overflow-y-auto no-scrollbar">
        ...
        <button
          ...
          className={`w-full text-left px-4 py-3.5 rounded-2xl transition-all flex items-center space-x-3 ${activeView === 'daily' ? 'bg-indigo-600 font-bold shadow-lg shadow-indigo-900/50' : 'hover:bg-white/5 text-indigo-200 font-medium'}`}
        >
