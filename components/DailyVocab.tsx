...
const DailyVocab: React.FC<DailyVocabProps> = ({ stats, setStats, words, isLoading, onAwardXP, onRecordAnswer, onLogMistake }) => {
  ...
  const [testQuestionWords, setTestQuestionWords] = useState<Record<string, string>>({}); // Maps Question ID -> Word string
  ...
  const startCumulativeTest = () => {
    ...
    const newWordMap: Record<string, string> = {};
    ...
    const questions: Question[] = selected.map((word, idx) => {
        ...
        const qId = `cumul-${Date.now()}-${idx}`;
        newWordMap[qId] = word.word;
    ...
    setTestQuestionWords(newWordMap);
    setTestIndex(0);
    setTestAnswers({});
    setTestSubmitted(false);
    setTestScore(0);
            ...
            testQuestions.length === 0 ? (
              ...
              <div className="max-w-3xl mx-auto space-y-8">
                 ...
                 {testQuestions.map((q, idx) => {
                   const relatedWord = testQuestionWords[q.id];
                   const isStarred = relatedWord ? starredSet.has(relatedWord) : false;

                   return (
                     <div key={q.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative">
                        {relatedWord && (
                           <button 
                              onClick={(e) => toggleStar(e, relatedWord)}
                              className="absolute top-8 right-8 p-2 rounded-full hover:bg-slate-100 transition-colors z-10"
                              title={isStarred ? "Unstar Word" : "Star Word"}
                           >
                              <svg className={`w-6 h-6 transition-colors ${isStarred ? 'text-yellow-400 fill-yellow-400' : 'text-slate-200 hover:text-slate-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                              </svg>
                           </button>
                        )}
